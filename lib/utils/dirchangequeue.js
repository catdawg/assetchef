...
/**
 * This class wraps a DirWatcher and smartly filters out events that are duplicates. 
...
module.exports = class DirChangeQueue {
    ...
    /**
     * @param {DirWatcher} dirWatcher the watcher for the directory
     * @throws {VError} if the directory is null or doesn't exist.
     */
    constructor(dirWatcher) {

        if (dirWatcher == null) {
            throw new VError("dirWatcher is null");
        }
    
        this._dirWatcher = dirWatcher;
        this._dirWatcher.on("dirchanged", this._onDirChange.bind(this));
        this._eventQueue = [];
    }
